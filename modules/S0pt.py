...
def pred(model, K, ch_l1, l1, l2, sep, min_d, max_d, min_meth, window, distances, do_plot):
    ...
    if do_plot:
        fig = plt.figure()
        plt.plot(g)
        plt.xticks(range(len(distances))[::10], distances[::10])
        plt.title("NNDE output")
        plt.show()
        plt.savefig('Fig/distance'+str(min_d)+'.png')
    if len(ds) == 0:
        return None, 0

    ch_l1_fit, ch_l2_guess = fit_simple(ch_l1, l1, l2, sep, min_d, max_d, min_meth, ds, window)
    return ch_l2_guess, ch_l1_fit, len(ds),ds,g
...
def opt_O2(guesses, lambs, Y, progress_list):
    ...
    amp = np.abs(amp)
    amp_err = 0.0001 * np.sum(amp)  #0.001
