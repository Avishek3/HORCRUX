...
sys.path.insert(0, "Users/avibaner/Downloads/Optml_avishek/modules")
...
def get_params_multi_procAOA(n_chans, max_n_paths, max_d, n_processes, min_d, min_n_paths, sep):
    '''
    helper function to speed up channel parameter generation.
    multiple processors are used and their results are combined.
    '''
    ...
    results = []
    for i in range(n_processes):
        results.append(p.apply_async(get_paramsAOA, args=(num_chans, max_n_paths, max_d, min_d, min_n_paths, sep)))
    p.close()
    p.join()
    output = [r.get() for r in results]
    params_list = []
...
def get_params(n_chans, max_n_paths, max_d, min_d, min_n_paths, sep):
    ...
    for i in range(n_chans):
        ...
        if sep != None:
            d_ns, a_ns, phi_ns, psi_ns = rfCommon.get_synth_params_sep(nps, min_d, max_d, sep)
        else:
            d_ns, a_ns, phi_ns, psi_ns = rfCommon.get_synth_params(nps, min_d, max_d)

        p = np.array(d_ns), np.array(a_ns), np.array(phi_ns), np.array(psi_ns)
        params.append(p)
    return params

def get_paramsAOA(n_chans, max_n_paths, max_d, min_d, min_n_paths, sep):
    params = []
    np.random.seed()
    for i in range(n_chans):
        nps = np.random.randint(min_n_paths, max_n_paths + 1)
        if sep != None:
            d_ns, a_ns, phi_ns, psi_ns, psi_angle = rfCommon.get_synth_params_sepAOA(nps, min_d, max_d, sep)
        else:
            d_ns, a_ns, phi_ns, psi_ns, psi_angle = rfCommon.get_synth_paramsAOA(nps, min_d, max_d)

        p = np.array(d_ns), np.array(a_ns), np.array(phi_ns), np.array(psi_ns), np.array(psi_angle)
        params.append(p)
    return params
