...
def get_model(in_dim, out_dim, h_units, num_h_layers, act_hid, optimizer, loss_fn, learning_, act_in=None, act_out=None,
              learning_rate=0.01):
    '''
    ...
    model.compile(optimizer, loss=loss_fn, metrics=['accuracy'])  # loss_fun
    return model
...
def get_callbacks(es_th=None, file_name=None, pat=None, min_del=None):
    ...
    if len(file_name) != 0:
        save_best = ModelCheckpoint(filepath=file_name, verbose=0, save_best_only=True, save_weights_only=False,
                                    period=1)
